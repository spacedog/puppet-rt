...
describe('rt::config') do
  ...
  context 'with default parameters from init' do
    ...
    it { is_expected.to contain_class('rt::config') }
    it do
      is_expected.to contain_file('/etc/rt').with(
        {
          'ensure' => 'directory',
          'owner'  => 'root',
          'group'  => 'root',
    ...
    it do
      is_expected.to contain_file('/etc/rt/RT_SiteConfig.pm').with(
        {
          'ensure' => 'present',
          'owner'  => 'apache',
          'group'  => 'apache',
...
",
    ...
    it do
      is_expected.to contain_exec('Site config syntax check')
    end
  end
end
